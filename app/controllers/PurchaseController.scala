...
class PurchaseController @Inject() (
  ...
  implicit val customLocalDateTimeWrites: Writes[LocalDateTime] =
    (o: LocalDateTime) => JsString(formatter.format(o))
  ...
  implicit val updateRequestReads: Reads[UpdateRequest] =
    Json.reads[UpdateRequest]

  def index(): Action[AnyContent] = userAction.async { userRequest: UserRequest[AnyContent] =>
    userRequest.user.map(process { user =>
      Ok(views.html.index(user)(userRequest.request))
    })
  }

  def load(): Action[AnyContent] =
    userAction.andThen(PermissionCheckAction).async(userAction.parser) { implicit request =>
  ...
  def update(): Action[JsValue] =
    userAction.andThen(PermissionCheckAction).async(parse.json) {
        ...
        userRequest.request.body
          ...
          .fold(
            ...
            updateRequest => {
              logger
                .info(
                  s"Start updating purchase: [id: ${updateRequest.id}, status: ${updateRequest.status}]"
                )
              purchaseRepository
                .update(updateRequest.id, updateRequest.status)
                .map(_ => Created)
            }
          )
    }
        ...
        userRequest.request.body
          ...
          .fold(
            ...
            items => {
              logger
                .info(
                  s"Start updating purchases: new: [${items.newItems}], delete: [${items.idsForDelete}]"
                )
              userRequest.user.flatMap(userOpt =>
                processPurchaseRequest(
                  items,
                  userOpt.getOrElse(throw new RuntimeException("System error"))
                )
                  .map(_ => Created)
              )
            }
  ...
  private def processPurchaseRequest(items: PurchaseRequest, user: User) = {
    val purchases = items.newItems.map { item =>
      Purchase(
        ...
        item.name,
        item.status,
        item.createdAt.getOrElse(LocalDateTime.now()),
        Some(user.id)
      )
    }
    Future.sequence(
